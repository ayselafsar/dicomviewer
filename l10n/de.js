OC.L10N.register(
    "dicomviewer",
    {
    "DICOM Viewer" : "DICOM-Bildbetrachter",
    "Digital Imaging for Medicine" : "Digitale Bildbearbeitung für die Medizin",
    "DICOM Viewer allows to display and manipulate DICOM images with a streamlined sidebar and viewer.\n\nSee [README](https://github.com/ayselafsar/dicomviewer) for a list of full features and future roadmap." : "Der DICOM-Viewer erlaubt das Anzeigen und bearbeiten von DICOM-Bilddateien mit der Hilfe einer optimierten Seitenleiste und Bildbetrachter.\n\nSiehe [README](https://github.com/ayselafsar/dicomviewer) für eine Übersicht aller Features und der Entwicklungsplan.",
    "DICOM Attributes" : "DICOM-Attribute",
    "Search for attributes…" : "Suche nach Attributen …",
    "Attribute" : "Attribute",
    "Value" : "Wert",
    "No DICOM attribute found" : "Kein DICOM-Attribut gefunden",
    "Unable to load the DICOM attributes" : "DICOM-Attribute konnten nicht geladen werden",
    "Open with DICOM Viewer" : "Mit DICOM Viewer öffnen",
    "Open All with DICOM Viewer" : "Alle Dateien mit dem DICOM Viewer öffnen",
    "There is no available DICOM image to display" : "Es existiert keine DICOM-Bilddatei zum Anzeigen",
    "Stack Scroll" : "Scrollen",
    "Levels" : "H/K",
    "Levels ROI" : "H/K Fenster",
    "Zoom" : "Zoom",
    "Pan" : "Schwenken",
    "Invert" : "Invertieren",
    "Flip H" : "Spiegeln H",
    "Flip V" : "Spiegeln V",
    "Rotate L" : "Rotieren L",
    "Rotate R" : "Rotieren R",
    "Magnify" : "Vergrößern",
    "Length" : "Länge",
    "Angle" : "Winkel",
    "Probe" : "Sensor",
    "Elliptical ROI" : "Ellipse",
    "Rectangle ROI" : "Rechteck",
    "Annotate" : "Notiz",
    "Clear" : "Löschen",
    "Reset" : "Reset",
    "Capture" : "Erfassen",
    "Enter your annotation" : "Notiz eingeben",
    "New label" : "Neue Bezeichnung",
    "Edit your annotation" : "Notiz bearbeiten",
    "OK" : "OK",
    "Remove Marker" : "Markierung löschen",
    "Capture High Quality Image" : "Hochauflösendes Bild erfassen",
    "Please specify the dimensions, and desired type for the output image." : "Bitte die Abmessungen und den Dateityp der gewünschten Bildausgabe angeben.",
    "Width (px)" : "Breite (px)",
    "Height (px)" : "Höhe (px)",
    "File Name" : "Dateiname",
    "File Type" : "Dateityp",
    "Show Annotations" : "Notizen anzeigen",
    "Image Quality (%)" : "Bildqualität (%)",
    "Image Preview" : "Bildvorschau",
    "Close" : "Schließen",
    "Download" : "Herunterladen",
    "Toggle Series Panel" : "Serienfenster einschalten",
    "Series" : "Serie",
    "More" : "mehr",
    "Close Viewer" : "Bildbetrachter schließen",
    "Loading…" : "Lade…",
    "Ser:" : "Serie:",
    "Img:" : "Bild:",
    "DICOM" : "DICOM",
    "Less" : "weniger",
    "Lossy: " : "Verlustbehaftet:",
    "Lossless / Uncompressed" : "Verlustfrei / Unkomprimiert",
    "WW/WC" : "H/K"
},
"nplurals=2; plural=(n != 1);");
