OC.L10N.register(
    "dicomviewer",
    {
    "DICOM Viewer" : "DICOM Viewer/библиотека с изображения/",
    "Digital Imaging for Medicine" : "Цифрови изображения за медицина",
    "DICOM Viewer allows to display and manipulate DICOM images with a streamlined sidebar and viewer.\n\nSee [README](https://github.com/ayselafsar/dicomviewer) for a list of full features and future roadmap." : "DICOM Viewer позволява да се показват и манипулират DICOM изображения с опростена странична лента и средство за преглед.\n\nВижте [README](https://github.com/ayselafsar/dicomviewer) за списък с пълни функции и бъдеща пътна карта. ",
    "DICOM Attributes" : "DICOM Атрибути",
    "Search for attributes…" : "Търсене на атрибути...",
    "Attribute" : "Атрибут",
    "Value" : "Стойност",
    "No DICOM attribute found" : "Няма намерен DICOM атрибут",
    "Unable to load the DICOM attributes" : "Не може да се заредят DICOM атрибути",
    "Open with DICOM Viewer" : "Отваряне с DICOM Viewer/инструмент за преглед/",
    "Open All with DICOM Viewer" : "Отваряне на всички с DICOM Viewer/инструмент за преглед/",
    "There is no available DICOM image to display" : "Няма налично DICOM изображение за показване",
    "Stack Scroll" : "Превъртнане на Група",
    "Levels" : "Нива",
    "Levels ROI" : "ROI Нива",
    "Zoom" : "Увеличаване",
    "Pan" : " Панорамно придвижване",
    "Invert" : " Инвертиране/Обръщане/",
    "Flip H" : " H Обръщане",
    "Flip V" : " V Обръщане",
    "Rotate L" : "Завъртане на L /ляво/",
    "Rotate R" : "Завъртане на R /дясно/",
    "Magnify" : "Увеличаване",
    "Length" : "Дължина",
    "Angle" : "Ъгъл",
    "Probe" : "Сонда",
    "Elliptical ROI" : "Елиптичен ROI",
    "Rectangle ROI" : "Правоъгълен ROI",
    "Annotate" : "Пояснение",
    "Clear" : "Изчистване",
    "Reset" : "Нулирай",
    "Capture" : " Заснемане",
    "Enter your annotation" : "Въведеждане на вашето пояснение",
    "New label" : "Нов етикет",
    "Edit your annotation" : "Редактиране на вашето пояснение",
    "OK" : "ОК",
    "Remove Marker" : "Премахване на маркер",
    "Capture High Quality Image" : "Заснемане на висококачествено изображение",
    "Please specify the dimensions, and desired type for the output image." : "Моля, посочете размерите и желания тип за изходното изображение.",
    "Width (px)" : "Ширина (px)",
    "Height (px)" : "Височина (px)",
    "File Name" : "Име на файл",
    "File Type" : "Тип файл",
    "Show Annotations" : "Показване на пояснения",
    "Image Quality (%)" : "Качество на изображение (%)",
    "Image Preview" : "Преглед на изображение",
    "Close" : "Затваряне",
    "Download" : "Свали",
    "Toggle Series Panel" : "Превключване на Панела на сериите",
    "Series" : "Серии",
    "More" : "Още",
    "Close Viewer" : "Затваряне на Viewer /библиотека с изображения/",
    "Loading…" : "Зареждане…",
    "Ser:" : "Сер:",
    "Img:" : "Изобр:",
    "DICOM" : "DICOM",
    "Less" : "По-малко",
    "Lossy: " : "Загуба: ",
    "WW/WC" : "WW/WC"
},
"nplurals=2; plural=(n != 1);");
