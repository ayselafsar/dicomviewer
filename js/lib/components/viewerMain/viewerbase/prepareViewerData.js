...
export const prepareViewerData = ({ studyInstanceUids, seriesInstanceUids, timepointId, timepointsFilter={} }) => {
    ...
    const promise = new Promise((resolve, reject) => {
        const processData = viewerData => {
            DCMViewer.viewer.studies.retrieveStudiesMetadata(viewerData.studyInstanceUids, viewerData.seriesInstanceUids).then(studies => {
                // Add additional metadata to our study from the studylist
                studies.forEach(study => {
                    const studylistStudy = DCMViewer.viewer.studylist.collections.Studies.findOne({
        ...
        if (studyInstanceUids && studyInstanceUids.length) {
            ...
            DCMViewer.viewer.studylist.timepointApi.retrieveTimepoints(timepointsFilter).then(() => {
...
const buildViewerDataFromTimepointId = timepointId => {
    const timepoint = DCMViewer.viewer.studylist.timepointApi.timepoints.findOne({ timepointId });
    if (!timepoint) {
...
const getDataFromTimepoint = timepoint => {
    ...
    const Timepoints = DCMViewer.viewer.studylist.timepointApi.timepoints;
    ...
    if (baseline) {
        ...
        DCMViewerLog.warn('No Baseline found while opening a Follow-up Timepoint');
    }

    const priorFilter = { latestDate: { $lt: timepoint.latestDate } };
    const priorSorting = { sort: { latestDate: -1 } };
    const prior = DCMViewer.viewer.studylist.timepointApi.timepoints.findOne(priorFilter, priorSorting);
